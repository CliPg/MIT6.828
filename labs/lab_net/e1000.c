...
e1000_transmit(struct mbuf *m)
{
  ...
  // 1. 检查网卡当前还有没有空闲的发送描述符用来发送新的数据包
  acquire(&e1000_lock);
  uint32 tdt = regs[E1000_TDT]; // 找到下一个要发送的发送描述符
  struct tx_desc *desc = &tx_ring[tdt];

  // 检查这个文件是否被发送
  if (!(desc->status & E1000_TXD_STAT_DD)) { // E1000_TXD_STAT_DD 是一个状态标志位，表示这个发送描述符已经被网卡处理完了
    release(&e1000_lock);
    // 网卡还没发完上一次的数据
    return -1;
  }
